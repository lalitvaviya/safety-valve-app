...
if 'project_log' not in st.session_state: st.session_state.project_log = [] # Stores full data now
...
def get_spring_from_file(file_name, orifice, set_pressure):
    # Mapping for your specific filenames
    actual_files = {
...
def render_datasheet_page(pdf_obj, data_pack):
    """Renders one datasheet onto the current page of pdf_obj"""
...
def generate_combined_pdf(all_logs):
    ...
    for log_item in all_logs:
        pdf.add_page()
        # Add a title for the Tag at the very top of the page if desired, or rely on the datasheet header
        # render the content
...
if st.button("🚀 Calculate & Generate Datasheet"):
    ...
    # Save data for Single PDF Logic
    ...
    # Pack full data for PDF regeneration
    full_data = {'proj': proj, 'proc': proc, 'fluid': fluid_d, 'mech': mech, 'res': res, 'safety': safe}
    
    # Generate single PDF bytes for immediate download
    pdf_b = generate_single_pdf(full_data)
    
    st.session_state.last_results = {"cap": disp_cap, "unit": unit_W, "orf": sel_orf, "spr": spring_txt}
    # Append to log with full data
    st.session_state.project_log.append({"Tag": tag_no, "Orifice": sel_orf, "PDF": pdf_b, "data": full_data})
